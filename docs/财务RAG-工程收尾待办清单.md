...
| 3 | **用户上下文持久化**（grill 定案：SQLite + 自建消息表 + 历史回显） | 🥉 P2 | 1.5-2 天 | 🔲 已定案待实施 |
...
- ✅ **全量重跑达标（2026-08-05 18:54）——§2 验收全过**：
  ...
  - **存证**：`backend/eval/run_all_report.json`（三层聚合）+ `retrieval_report.json`（检索明细）——答辩数据
...
P2 按时间余量
  ...
  └─→ ③ 用户上下文持久化（1.5-2 天）—— 答辩演示"持久记忆"加分项
⏸️ LlamaIndex 暂缓（不排期）
```

> 排期原则：**答辩/面试优先**——MCP 讲的是协议工程能力，评测自动化讲的是工程素养，二者都是"可讲 5 分钟"的素材；用户上下文持久化是加分项。
