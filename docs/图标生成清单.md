...
### 17. 行业基准 — `icon-benchmark.svg`（2026-08-06 新增）

- **用途**：左侧资料库「行业基准」入口 + 折叠态图标
- **画法**：柱状图（三条高度递增的柱子 + 基线）
- **Prompt**：`Minimal bar chart icon, three ascending bars on a baseline, line art style, 2px stroke, design primary blue (#014DB2), transparent background, rounded caps.`
