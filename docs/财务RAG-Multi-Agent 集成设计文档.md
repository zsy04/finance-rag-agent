...
| P8 | **v1.1 成本可接受** | 接受 +20-40% 延迟 / 1.5-2× token，补三项补偿（子 Agent 全局单例 / 主 prompt 精简抵消 / 演示话术兜底） | §5.3 单例设计 + §9 话术 |
...
- **记忆隔离叙事**：子 Agent 无状态设计是有意为之——"画像独立存储 + 子 Agent 无历史"，天然防上下文污染，是演示可展开的设计点
...
### 4.2 阶段二：完整版 — Supervisor 自建（可选，演示前有余量再做）
...
| B | 成本取舍 | +20-40% 延迟 / 1.5-2× token 是否可接受 | **接受 + 三项补偿**（子 Agent 全局单例 / 主 prompt 精简抵消 / 演示话术兜底） | §5.4 单例 + §9.1 话术 |
