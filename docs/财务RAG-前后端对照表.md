...
> **更新**：2026-07-30 — 对齐 LangChain v1 Agent 重构后的实际接口
...
|---------|------------|-------------|----------------|
| `thinking` | Agent 开始处理 / 工具调用中 | `{"message": "正在为您处理……"}` 或 `{"tool": "工具名"}` | 显示加载提示 / 工具执行中动画 |
| `step` | LLM 逐 token 生成 | `{"content": "你好"}` | 追加流式文字到 AI 气泡末尾 |
...
| `error` | 处理失败 | `{"content": "错误消息"}` | 红色边框气泡 + `[🔄 重试]` 按钮 |
